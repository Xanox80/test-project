...
VillaRouter.route('/api/create').post(async (req, res) => {
	const { nameVilla, surnameVilla, price, residence } = req.body; // Extract title and content from request body

	const result = await newVilla.createVilla(nameVilla, surnameVilla, price, residence);
	res.send(result);
});
...
VillaRouter.route('/api/getPhotoVilla/:id').get(async (req, res) => {
	const result = await newVilla.getPhotoVilla();
	res.send(result);
});
